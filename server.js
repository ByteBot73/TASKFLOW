...
// console.log('FRONTEND_URL:', process.env.FRONTEND_URL ? 'Loaded' : 'NOT LOADED (using fallback)');
// console.log('-----------------------------------------');
...
// const DISCORD_REDIRECT_URI = process.env.DISCORD_REDIRECT_URI; // Pulled from .env or Render configs
// const DISCORD_SCOPES = 'identify'; // We only need 'identify' for basic user info (Discord ID)

// // Define the frontend URL, defaulting to localhost for local development
...
//                 return res.redirect(`${FRONTEND_URL}/?discord_connect_status=success&message=${encodeURIComponent(message)}&token=${newKanbanToken}&discord_username=${encodeURIComponent(discordUsername)}&discord_discriminator=${encodeURIComponent(discordDiscriminator)}`);
...
//                 return res.redirect(`${FRONTEND_URL}/?discord_connect_status=info&message=${encodeURIComponent(message)}&discord_username=${encodeURIComponent(discordUsername)}&discord_discriminator=${encodeURIComponent(discordDiscriminator)}`);
//             }
//         }

//     } catch (error) {
//         console.error('Discord OAuth callback error:', error);
//         res.redirect(`${FRONTEND_URL}/?discord_connect_status=error&message=${encodeURIComponent(error.message || 'An unknown error occurred during Discord connection.')}`);
//     }
// });
...
console.log('DISCORD_REDIRECT_URI:', process.env.DISCORD_REDIRECT_URI || 'Not Set, using default');
console.log('JWT_SECRET:', process.env.JWT_SECRET ? 'Loaded' : 'NOT LOADED');
console.log('FRONTEND_URL:', process.env.FRONTEND_URL ? 'Loaded' : 'NOT LOADED (using fallback for localhost)');
console.log('KANBAN_BOT_API_KEY:', process.env.KANBAN_BOT_API_KEY ? 'Loaded' : 'NOT LOADED (REQUIRED for bot admin functions)'); // New check for the bot API key
console.log('-----------------------------------------');
...
// Import Mongoose Models from separate files (assuming these paths are correct relative to server.js)
const UserSchema = new mongoose.Schema({
    username: { type: String, required: true, unique: true },
    passwordHash: { type: String, required: true },
...
const Category = require('./models/category'); // Assuming ./models/category.js
const Card = require('./models/card'); // Assuming ./models/card.js
...
// NEW: Define the Bot API Key from environment variables. This must match the bot's key.
const KANBAN_BOT_API_KEY = process.env.KANBAN_BOT_API_KEY;
if (!KANBAN_BOT_API_KEY || typeof KANBAN_BOT_API_KEY !== 'string' || KANBAN_BOT_API_KEY.length < 32) {
    console.warn("WARNING: KANBAN_BOT_API_KEY environment variable is missing, not a string, or too short. Bot-to-backend admin functions may not work.");
}
...
// --- Authentication Middleware for Frontend Users ---
const authenticateToken = (req, res, next) => {
    const authHeader = req.headers['authorization'];
    if (!authHeader) {
        return res.status(401).json({ message: 'Authentication token required.' });
...
// NEW: Middleware for Discord Bot Authentication using API Key
const authorizeBot = (req, res, next) => {
    const apiKey = req.headers['x-kanban-bot-api-key']; // Expecting the custom header from the bot
    if (!apiKey || apiKey !== KANBAN_BOT_API_KEY) {
        console.warn('Unauthorized bot API key attempt:', apiKey);
        return res.status(403).json({ message: 'Unauthorized: Invalid bot API key.' });
    }
    next();
};
...
// --- NEW ADMIN API ROUTES (Protected by Bot API Key) ---

// Get summary information for a specific user
app.get('/api/admin/user/:userId/summary', authorizeBot, async (req, res) => {
    try {
        ...
        // 2. Count categories for the user
        const categoryCount = await Category.countDocuments({ userId: userId });

        // 3. Count cards for the user
        const cardCount = await Card.countDocuments({ userId: userId });

        // 4. Determine Discord connection status
        ...
        res.status(200).json({
            ...
            // Provide default 'N/A' for discordUsername and discordDiscriminator if not connected
            discordUsername: user.discordUsername || 'N/A',
            discordDiscriminator: user.discordDiscriminator || 'N/A',
        ...
        console.error('Error fetching user summary for admin:', error);
        res.status(500).json({ message: 'Internal server error', error: error.message });
    }
});
...
app.listen(port, () => {
  ...
  console.log(`Frontend accessible via public/index.html`);
  console.log(`Discord OAuth Redirect URI: ${DISCORD_REDIRECT_URI}`);
  console.log(`Frontend Base URL for Redirects: ${FRONTEND_URL}`);
});
